...
#                 # print(query_pred.cpu().numpy().shape)

#                 query_pred = query_pred.argmax(dim=1)[0].cpu().numpy()
...
#                         # print(_qimgs.shape)
#                         _lb_buffer[_scan_id] = [_pred.transpose(2,0,1),_labels.transpose(2,0,1),_qimgs.transpose(3,1,2,0), _means.transpose(3,1,2,0), _stds.transpose(3,1,2,0)] # H, W, Z -> to Z H W
#                     else:
#                         lb_buffer[_scan_id] = [_pred, _labels, _qimgs, _means, _stds]
...
#                 gif_frames = []
#                 _pred, _label, _qimg, _mean, _std = item
#                 print(_pred.shape, _qimg.shape, _label.shape, _mean.shape, _std.shape)
#                 # print(_pred)
...
#                     fid = os.path.join(f'{_run.observers[0].dir}/interm_preds', f'scan_{_scan_id}_label_{curr_lb}_{i}.png')
#                     # sitk.WriteImage(itk_pred, fid) #, True)
#                     qim = _qimg[i]*_std[i] + _mean[i]
#                     qim = 255*(qim - qim.min())/(qim.max() - qim.min())
...
#                     gif_frames.append(qim)
#                     qim.save(fid)
                    
#                 fid = os.path.join(f'{_run.observers[0].dir}/interm_preds', f'scan_{_scan_id}_label_{curr_lb}.gif')
