    ...
    with torch.no_grad():
        ...
        for curr_lb in test_labels:
            ...
            for sample_batched in testloader:
                ...
                query_pred, _, _, _ = model( sup_img_part , sup_fgm_part, query_images, query_labels) #, isval = True, val_wsize = _config["val_wsize"] )
                ...
                if (sample_batched["z_id"] - sample_batched["z_max"] <= _config['z_margin']) and (sample_batched["z_id"] - sample_batched["z_min"] >= -1 * _config['z_margin']):
                    mar_val_metric_node.record(query_pred[0], np.array(query_labels[:,0,:,:].cpu()), labels=[curr_lb], n_scan=curr_scan_count) 
                else:
                    pass
