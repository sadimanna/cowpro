...
class SuperpixelDataset(BaseDataset):
    ...
    def transform_img_lb(self, comp, c_label, c_img, use_onehot, nclass, num_rep, **kwargs):
        ...
        label_remove = False
        # if num_rep%2==1 and random.random() < 0.4:
        #     # t_img, t_label = self.random_erasing(t_img, t_label)
        #     t_img = cv2.inpaint(t_img, np.uint8(t_label),3,cv2.INPAINT_TELEA)
        #     t_label = np.zeros_like(t_label)
        #     label_remove = True

        # if not label_remove and num_rep%2==1:
        #     if random.random() < 0.25:
        #         t_img, t_label = self.random_cutmix(t_img, t_label)
    ...
    def __getitem__(self, index):
        ...
        for ii in range(self.num_rep):
            ...
            # -----------------------------------------
            # RANDOMLY MAKE THE SUPPORT MASK COMPLETELY ZERO
            # if not label_remove:
            #     if random.random()<0.25 and ii%2==0:
            #         lb = np.zeros_like(lb)
            #         support_flag = False
            #     # IF SUPPORT MASK IS ZERO, THEN MAKE QUERY MASK ZERO
            #     if not support_flag and ii%2==1:
            #         lb = np.zeros_like(lb)
            #########################################################
            if img.ndim == 2:
                img = img[:,:,None]
            ...
            # print(img.shape)
            lb  = torch.from_numpy( lb.transpose((2,0,1)))#.squeeze(-1))
            # print(lb.shape)
