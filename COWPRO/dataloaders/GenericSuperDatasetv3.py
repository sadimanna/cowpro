...
class SuperpixelDataset(BaseDataset):
    ...
    def organize_sample_fids(self):
        ...
        for curr_id in self.scan_ids:
            ...
            # _lb_fid  = os.path.join(self.base_dir, f'superpix-{self.superpix_scale}_{curr_id}.nii')
            ##########################################
    ...
    def get_bg_label(self, g):
        g_ = g.copy()
        bg_labels = []
        comps = nx.connected_components(g_)
        for i, nodes in enumerate(comps):
            for node in g_.nodes:
                if g_.nodes[node]['mean color'][0] < self.dataset_config['fg_thresh']:
    ...
    def cut_thresh(self,g, labels1, thresh):
        ...
        g_ = g.copy()
        bg_labels = self.get_bg_label(g_)
        to_remove = []
        to_merge = []
        # print('here')
        for x, y, d in g_.edges(data=True):
            ...
            is_x_bg = g_.nodes[x]['mean color'][0] < self.dataset_config['fg_thresh']
            is_y_bg = g_.nodes[y]['mean color'][0] < self.dataset_config['fg_thresh']
    ...
    def supcls_pick_binarize(self, image_t): #super_map, sup_max_cls, bi_val = None): # 
        ...
        labels1 = segmentation.felzenszwalb(dst, scale = 1.0, sigma = 0.8, min_size = 250, channel_axis = None)
        ...
        labels2 = self.cut_thresh(g, labels1, thresh = 10)
        ...
        if label_choices == []:
            ...
            self.sup_max_cls = len(label_choices)
        ...
        # ax[1].set_title('Seg Output')

        # ax[2].imshow(out2, cmap='gray')
        # ax[2].set_title('merge output')
        # plt.show()
        # print(labels2)
        random_label = random.choice(np.unique(label_choices))
    ...
    def __getitem__(self, index):
        ...
        try:
            label_t = self.supcls_pick_binarize(image_t) #label_raw, sup_max_cls) #image_t)  #comment out for agun model
            if self.sup_max_cls < 1:
                # print(label_t)
