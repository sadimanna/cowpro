...
def main(_run, _config, _log):
    ...
    model = AGUNet(cfg=_config['tversky_params'], is_val = True)
        ...
        for sup_idx in range(len(_scan_ids)):
            ...
            for curr_lb in test_labels:
                ...
                support_batched = te_parent.get_support(curr_class = curr_lb, 
                                                        class_idx = [curr_lb], 
                                                        scan_idx = [sup_idx], #_config["support_idx"], 
                                                        npart=_config['task']['npart'])
                ...
                for sample_batched in testloader:
                    ...
                    # print(query_labels[0][0].sum(), query_labels[0][0].shape, np.unique(query_labels[0][0].cpu()))
                    ...
                    query_pred, _, _, _, _, _, _ = model(sup_img_part , sup_fgm_part, query_images, query_labels) #, isval = True, val_wsize = _config["val_wsize"] )

                    # print(query_pred.cpu().numpy().shape,query_labels.cpu().numpy().shape)
                    # print(query_labels[0][0].sum(), query_labels[0][0].shape, np.unique(query_labels[0][0].cpu()))
                    # print(query_pred.min(), query_pred.max()) #/3)**0.5)
                    query_pred = query_pred.cpu().numpy() 
                    # query_pred = query_pred / query_pred.max() #
                    query_pred = (query_pred > 0.5).astype(np.float32)
                    # print(np.sum(query_pred))
                    _pred[..., ii] = query_pred[0].copy()
                    _labels[..., ii] = query_labels[0][0].detach().cpu().clone().numpy()
                    # print(_labels[..., ii].sum(), _labels[..., ii].shape, np.unique(_labels[..., ii]))
                    ...
                    supfgs[..., ii] = sup_fgm_part[0][0][0].cpu().numpy()[0]
                    ...
                    if (sample_batched["z_id"] - sample_batched["z_max"] <= _config['z_margin']) and (sample_batched["z_id"] - sample_batched["z_min"] >= -1 * _config['z_margin']):
                        mar_val_metric_nodes[_scan_ids[sup_idx]].record(query_pred[0], np.array(query_labels[:,0,:,:].cpu()), labels=[curr_lb], n_scan=curr_scan_count) 
                    else:
                        pass
                ...
                # print(list(save_pred_buffer.keys()))
